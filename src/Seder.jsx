...
function Sedarim({d, t}){
    ...
    function onDragEnd(result){
        ...
        let newSedarim = [...sedarim];
        const[removed] = newSedarim.splice(result.source.index, 1);
        newSedarim.splice(result.destination.index, 0, removed);
        setSeder(newSedarim);
    }
    return (
                ...
                <DragDropContext onDragEnd={onDragEnd}>
                    <Droppable droppableId={crypto.randomUUID()}>
                        {(provided, snapshot) =>(
                            ...
                            >
                                {sedarim.map((seder, index) => (
                                    <Draggable draggableId={seder.id} index={index}>
                                        {(provided, snapshot) => (
                                        <div class="flex bg-[#0B3560] flex-row justify-between border-1 rounded-md hover:bg-blue-900" 
                                            key={seder.id}
                                            ref={provided.innerRef}
                                        ...
                                        >
                                            ...
                                            <button class="bg-white flex-no-shrink p-1 ml-2 border-1 rounded text-black border-red opacity-100 " aria-label="Close alert" type="button" onClick={() => removeSeder(seder.id)}>
