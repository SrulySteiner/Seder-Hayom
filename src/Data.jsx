...
function collectTitlesDeep(value, out){
  if (!value) return;
  if (Array.isArray(value)){
    for (const v of value) collectTitlesDeep(v, out);
    return;
  }
  if (typeof value === 'object'){
    if (typeof value.title === 'string' && value.title.trim()) out.push(value.title.trim());
    // Common shape keys on Sefaria responses; also walk remaining keys for nested nodes.
    const prioritizedKeys = ['children', 'nodes', 'contents', 'items', 'toc', 'categories'];
    for (const k of prioritizedKeys){
      if (value[k]) collectTitlesDeep(value[k], out);
    }
    for (const [k, v] of Object.entries(value)){
      if (prioritizedKeys.includes(k) || k === 'title') continue;
      collectTitlesDeep(v, out);
    }
  }
}
...
function dataComp({d, t, s}) {
  ...
  const [startDate, setStartDate] = useState(new Date(d.currentDate));
  const [endDate, setEndDate] = useState(new Date(d.currentDate));
  ...
  const getSederName = (name, metric, dayStart, dayEnd) => {
    let result = name;
    if(metric == index[0]){
      result += ' ' + metric + 's' + ' ' + dayStart + '-' + dayEnd;
    }
    else{
      let i = 0;
      while(dayStart > 0){
        if(length[i] > dayStart){
          dayStart -= length[i];
          i++;
        }
      }
      ...
      while (loop.getDate() <= endDate.getDate() && dayStart <= totalLength) {
        ...
        let newTask = {
          ...
          name: //newSeder.name + ' ' + metric + ' ' + dayStart + '-' + dayEnd,
            getSederName(newSeder.name, metric, dayStart, dayEnd),
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const titles = [];
        collectTitlesDeep(data, titles);
        const nextTexts = Array.from(new Set(titles)).filter(Boolean).sort((a,b)=>a.localeCompare(b));
        setTexts(nextTexts);
        setText((prev) => (prev && nextTexts.includes(prev)) ? prev : (nextTexts[0] || ''));
        setLoading(false);
      } catch (error) {
        setError(error);
        setLoading(false);
  ...
  return (
   <div class="flex flex-col flex-row justify-center">
    ...
    <label>Pick a Text:
      <select onChange={changeText} value={text}>
        {texts.map((t) =>{
          return <option key={t}>
                  {t}
                 </option>
